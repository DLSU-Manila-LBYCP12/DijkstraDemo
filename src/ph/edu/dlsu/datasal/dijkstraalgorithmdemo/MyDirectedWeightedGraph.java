 ...
 */
package ph.edu.dlsu.datasal.dijkstraalgorithmdemo;
 ...
 * @author cobalt
 */
public class MyDirectedWeightedGraph {
    ...
    // Constructor
    public MyDirectedWeightedGraph(int numOfVertices) {
    ...
    private void nodeExistenceCheck(String from, String to) {
        if (!vertices.contains(from) || !vertices.contains(to)) {
            throw new InvalidNodeRefException("ERROR: Either Node " + from + " or Node " + to + " does not exist");
        }
    }

    private void keyExistenceCheck(String key) {
        if (!vertices.contains(key)) {
            throw new InvalidNodeRefException("ERROR: Node " + key + " does not exist");
        }
    }
